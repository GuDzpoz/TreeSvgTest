...
define(["ajax"], function(ajax) {
    function initMenu(list) {
        ...
        for(index in list) {
            ...
            var tr = createTr(element.title, element.title);
            table.appendChild(tr);
        }
        body.appendChild(table);
    }
    
    function createTr(title, message) {
        var tr = document.createElement("tr");
        
        var td1 = document.createElement("td");
        var a = document.createElement("a");
        a.setAttribute("href", "browse.html?" + encodeURIComponent(message));
        a.innerHTML = title;
        td1.appendChild(a);
        tr.appendChild(td1);
    ...
    ajax.send(ajax.requests.GET_LIST, {}, function(error, xmlhttp) {
        if(error) {
            ajax.simpleAlert(xmlhttp);
        }
        else {
